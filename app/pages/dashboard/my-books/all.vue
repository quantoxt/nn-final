        ...
        <div v-else class="card-wrap">
            <div v-for="book in books" :key="book.id"
                ...
                <div class="flex items-center justify-center flex-wrap gap-2">
                    <p class="text-gray-400 flex items-start justify-start">{{ book.categories?.name }}</p> •
                ...
                <div class="flex items-center gap-3 justify-center">
                    <template v-if="!['rejected', 'archived'].includes(book.status)">
                        <button
                            ...
                            @click="$router.push(`/dashboard/edit/${book.slug}`)">Edit</button>
                    </template>
                    <template v-if="!['rejected', 'archived'].includes(book.status)">
                        <button
                            class="bg-amber-600 cursor-pointer px-2 py-0.5 rounded-sm text-white text-semibold text-sm"
                            @click="$router.push(`/dashboard/edit/${book.slug}-add-chapter`)">Add Chapter</button>
                    </template>
